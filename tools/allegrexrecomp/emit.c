...
static void emit_imports(FILE *f, const a_analysis *an, const emit_opts *o) {
    fprintf(f,
        "/* Generated by allegrexrecomp -- do not edit.\n"
        " *\n"
        " * Default implementations for the %d firmware functions this module\n"
        " * imports. Each traps with its stub address so an unimplemented call is\n"
        " * named at the moment it happens.\n"
        " *\n"
        " * These are deliberately weak-by-convention: the HLE layer replaces them\n"
        " * one at a time as sceKernel/sceGu/sceCtrl come up, and whatever is not\n"
        " * yet implemented keeps reporting itself. Bringing a game up is largely\n"
        " * the process of watching this list stop being hit.\n"
        " */\n"
        "#include \"%s_funcs.h\"\n\n",
    ...
    for (int i = 0; i < an->nimports; i++)
        fprintf(f,
            "void psp_import_%08X(void) { psp_unimplemented(0x%08Xu, \"import\"); }\n",
